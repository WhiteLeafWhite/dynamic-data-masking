...
int AccountManager::Login(const std::string &name, const std::string &password){
  std::cout<<"AM.cpp:   "<<name<<"##"<<password<<std::endl;
  if(!accountInfos.count(name)) return -1; // 如果没有这个用户，那么就直接返回false
...
int AccountManager::Login(const std::string &clientMessage){

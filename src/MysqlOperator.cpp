...
MysqlOperator::MysqlOperator(const char* dbHost, const char* dbUser, const char* dbPass, 
    const char* dbName, unsigned int dbPort):DB_HOST(dbHost), DB_USER(dbUser), DB_PASS(dbPass), DB_NAME(dbName)
     , DB_PORT(dbPort)
{
...
std::vector<std::vector<std::string> > MysqlOperator::execute(const std::string& query){
    //用到的变量
